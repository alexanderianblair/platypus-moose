...
private:
  ...
  template <typename T>
  MFEM_HOST_DEVICE inline static auto coeff(const mfem::future::tensor<T, dim> & a)
  {
...
public:
  ...
  struct MFApply
  {
    ...
    //        (\nabla_{\xi} v) d\xi $
    MFEM_HOST_DEVICE inline auto operator()(const mfem::future::tensor<dscalar_t, dim> & dudxi,
...
private:
  ...
  public:
    DifferentiableEquationSystemJacobian(const DifferentiableEquationSystem * minsurface,
                                         const mfem::Vector & x)
      : Operator(minsurface->Height()), minsurface(minsurface), z(minsurface->Height())
    {
      minsurface->u.SetFromTrueDofs(x);
      auto mesh_nodes =
          static_cast<mfem::ParGridFunction *>(minsurface->H1.GetParMesh()->GetNodes());
...
public:
  DifferentiableEquationSystem(mfem::ParFiniteElementSpace & H1, const mfem::IntegrationRule & ir)
    : Operator(H1.GetTrueVSize(), H1.GetTrueVSize()), H1(H1), ir(ir), u(&H1)
  {
    mfem::Array<int> all_domain_attr(H1.GetMesh()->attributes.Max());
    all_domain_attr = 1;
    H1.GetParMesh()->EnsureNodes();
    auto & mesh_nodes = *static_cast<mfem::ParGridFunction *>(H1.GetParMesh()->GetNodes());
    ...
    auto input_operators = mfem::future::tuple{mfem::future::Gradient<SOLUTION_U>{},
    ...
    auto output_operators = mfem::future::tuple{mfem::future::Gradient<SOLUTION_U>{}};
    ...
    res->AddDomainIntegrator(
        mf_apply_qf, input_operators, output_operators, ir, all_domain_attr, derivatives);
